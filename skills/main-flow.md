...
The main flow handles living dialogue, coordination, priorities, authority decisions, evidence review, synthesis, and straightforward operational sends on an already-known native route. Delegate bounded inspection, test, and implementation work through this harness's own subagent call.
...
Delegate new file inspection and locating to a small read-only subflow; review the returned relevant content and source location yourself. Locating includes listing a directory, searching git or jj history, and grepping an index. The main flow runs a shell command only for `flow-id`, direct authorized native-route sends, and the writes it owns.
*Subflow scripts.* When a locate, probe, or read-tail task recurs, it is a subflow script: a subflow with a registered name, a fixed brief, a fixed return shape, and explicit noise-filter rules. The main flow invokes the script by name and passes only its arguments; the script keeps every id, path and hash inside itself and returns only the semantic outcome. Subflow scripts are the standard way the main flow reaches through the harness when its own direct primitive is not the applicable route — see the `subflow-scripts` skill for the current catalogue.
...
{% endif %}
Before any native launch is treated as a main flow, the launcher composes and submits one complete first user prompt containing every startup-only skill and the launch brief. Their internal order carries no contract. The launcher reads the native transcript back and proves that one complete first user prompt was accepted and contains the selected startup skills and brief before readiness. The living never types a startup command. A process or permission mode that cannot inject and verify this prompt is not a launcher. If the receipt proves that one startup-only skill was omitted, the launcher may inject that skill as an explicit repair and verify its native expansion; ambiguity never permits a resend.
